...
A helper script, `launch.py`, is provided to simplify execution. This script wraps the `nextflow run` command with Biowulf-specific settings and automatically configures output directories, logging, and profiles.
...
??? "launch.py"
    ...
    def collapse_unique(vals: List[str], label: str) -> str:
        ...
        if len(vals) > 1:
            sys.exit(f"ERROR: Multiple unique values found for '{label}' in samplesheet: {', '.join(vals)}")
        return vals[0]
    ...
    # ----------------- CLI -----------------
    def parse_args():
        p = argparse.ArgumentParser(description="Submit the Oncogenomics Nextflow workflow to SLURM.")

        # Mutually exclusive source of samplesheet
        src = p.add_mutually_exclusive_group(required=True)
        src.add_argument("--samplesheet", help="Path to existing samplesheet CSV")
        src.add_argument("--patient", help="Patient ID (requires --casename to build a samplesheet)")
        ...
        p.add_argument("--genome",   default=DEFAULT_GENOME, choices=sorted(ACCEPTED_GENOMES),
        ...
        p.add_argument("--no-resume", action="store_true", help="Run Nextflow without -resume")
    ...
    def main():
        ...
        if genome not in ACCEPTED_GENOMES:
            sys.exit(f"Invalid genome specified. Accepted values: {', '.join(sorted(ACCEPTED_GENOMES))}")
        ...
        if args.samplesheet:
            ...
            casename = collapse_unique(read_unique_values(samplesheet, "casename"), "casename")
        else:
            ...
            print("🔧 Generating samplesheet via:\n  " + " ".join(map(shlex.quote, builder_cmd)))
        ...
        if args.profile:
            ...
            profile = "biowulf_test_run_slurm" if genome == "hg19" else "biowulf_mouse_RNA_slurm"
    ...
    """
        ...
        # Submit
        try:
            proc = subprocess.run(["sbatch"], input=sbatch_script, text=True, check=True, capture_output=True)
        except subprocess.CalledProcessError as e:
...
!!! tip "mm39"
If mm39 is selected for the genome, the pipeline will run _Mapping & Gene expression_ only for mouse data.
