...
for row in samplesheet_data:
    if row["type"] == "xeno_RNA":
        row["type"] = "tumor_RNA"
    # NOTE: removed the line that replaced commas with hyphens in Matched_RNA
...
for row in samplesheet_data:
    ...
    if row["type"] in ["tumor_DNA", "cell_line_DNA"]:
        ...
        if matched_rna and not matched_normal:
            ...
            matched_rnas = (
                [rna.strip() for rna in matched_rna.split(",") if rna.strip()]
                if matched_rna
                else []
            )
...
for row in samplesheet_data:
    ...
    if (
        sample_casename_counts[(sample, casename)] > 1
        and row_type in ["tumor_DNA", "cell_line_DNA"]
    ...
    if sample_casename_counts[(sample, casename)] > 1 and row_type in [
        ...
        for other_row in samplesheet_data:
            matched_rnas = (
                [
                    rna.strip()
                    for rna in other_row["Matched_RNA"].split(",")
                    if rna.strip()
                ]
