process RECONCNV {
    ...
    if [ ! -f ${bn}.html ];then
    	...
    	reconCNV.py -r \${cleaned_cnr} -g \${cleaned_cnr} -x ${recon_data_dir}/${params.genome_v}_genome_length_chr.txt -a ${recon_data_dir}/${params.genome_v}_COSMIC_genes_model_chr.txt -s \$cns -c ${recon_config_file} -o ${bn}.html -d ./
    	chrs=()
    	...
    	for chr in "\${chrs[@]}";do
    		chr_cnr=${bn}.\${chr}.cnr
    		chr_cns=${bn}.\${chr}.cns
        chr_len="${recon_data_dir}/${params.genome_v}_genome_length_\${chr}.txt"

        genome_len=${recon_data_dir}/${params.genome_v}_genome_length_chr.txt
            ...
            reconCNV.py -r \${chr_cnr} -g \${chr_cnr} -x \${chr_len} -a ${recon_data_dir}/annotation/${params.genome_v}/\${chr}.annotation.txt -s \${chr_cns} -c ${recon_config_file} -o ${bn}.\${chr}.html -d ./
    	done
    fi
